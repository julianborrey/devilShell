...
void cycleThroughEachJob(job_t* firstJob){
  ...
  while(currentJob != NULL){ //while not at end of list
     if(!builtin_cmd(job_t *last_job, int argc, char **argv)){ //for process

        if(jobInBackground(currentJob)){ //check where we will run
          spawn_job(currentJob, false);  //for background
        } else {
          spawn_job(currentJob, true);   //for foreground
        }
